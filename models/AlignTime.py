...
class AlignTime(nn.Module):
    ...
    def pad_time_series(self, x):
        ...
        start_period = self.highest_freq_period * (seq_len // self.highest_freq_period-1) - 1
        end_period = start_period + self.highest_freq_period
        last_full_period_data = x[:, start_period:end_period, :]
        pad_data = last_full_period_data[:, -pad_len:, :]
        x_padded = torch.cat([x, pad_data], dim=1)
