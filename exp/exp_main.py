...
from models import AlignTime
from utils.tools import EarlyStopping, adjust_learning_rate, visual
from utils.metrics import metric, R2

warnings.filterwarnings('ignore')

non_transformer = ['AlignTime']

class Exp_Main(Exp_Basic):
    def __init__(self, args):
        super(Exp_Main, self).__init__(args)

    def _get_highest_freq_period(self):
        train_data, train_loader = self._get_data(flag='train')
        periods = []
        for data in train_loader:
            lookback_window = data[0]
            frequency_list = abs(torch.fft.rfft(lookback_window, dim=1)).mean(0).mean(-1)
            frequency_list[0] = 0
            top_frequency, top_list = torch.topk(frequency_list, 1)
            top_list = top_list.detach().cpu().numpy()
            period = lookback_window.shape[1] // (top_list)
            periods.append(period.item())

        freq_period = torch.tensor(periods)
        highest_freq_period = torch.mode(freq_period).values.item()
        print("Highest frequency period:", highest_freq_period)
        return highest_freq_period
    ...
    def _build_model(self):
        ...
        self.args.highest_freq_period = self._get_highest_freq_period()
        model = model_dict[self.args.model].AlignTime(self.args).float()
        ...
        f.write('Number of parameters:{:.4f} M'.format(total / 1e6))
        f.write('\n')
        f.close()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if (i + 1) % 100 == 0:
                    ...
                    f = open("result_long_term_forecast.txt", 'a')
                    f.write("\titers: {0}, epoch: {1} | loss: {2:.7f}".format(i + 1, epoch + 1, loss.item()))
                    f.write('\n')
                    f.close()
    ...
    def test(self, setting, test=0):
        ...
        f = open("result_long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{:.4f}, mae:{:.4f}, R2:{:.4f}'.format(mse, mae, r_squared))
        f.write('\n')
        f.close()
